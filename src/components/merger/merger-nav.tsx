...
import { GitMerge, Save, Download, ChevronLeft, Check, Loader2 } from "lucide-react"
import { useMergerStore } from "@/store/merger-store"
import { toast } from "sonner"
import { cn } from "@/lib/utils"

export function MergerNav() {
  const { analysisName, setAnalysisName, saveStatus, isDirty, isDemo, persistToStorage } = useMergerStore()
  ...
  async function handleSave() {
    ...
    setSaving(true)
    useMergerStore.getState().setSaveStatus("saving")
    persistToStorage()
    await new Promise(r => setTimeout(r, 500))
    useMergerStore.getState().setSaveStatus("saved")
    useMergerStore.getState().setLastSavedAt(new Date())
    setSaving(false)
    toast.success("Fusión guardada")
  }
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-border">
      <div className="h-14 px-4 flex items-center gap-2">
        ...
        <div className="w-6 h-6 bg-purple-600 rounded flex items-center justify-center shrink-0">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          {editingName && !isDemo ? (
            <input
              autoFocus
              className="text-sm font-semibold text-foreground bg-transparent border-b border-purple-600 outline-none w-full max-w-xs"
              value={analysisName}
            ...
            <button
              ...
              className={cn(
                ...
                isDemo ? "text-foreground cursor-default" : "text-foreground hover:text-purple-600"
              )}
            >
              {analysisName}
          ...
          <span className="shrink-0 text-[10px] font-semibold bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full">Fusión</span>
          {isDemo && (
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={() => toast.info("Export — próximamente")}
            className="inline-flex items-center gap-1.5 h-8 px-3 bg-purple-600 text-white text-sm font-medium rounded-lg hover:bg-purple-700 transition-colors"
          >
